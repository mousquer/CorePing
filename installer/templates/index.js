...
// Templates
const tplConfig = require('./templates/config');
const tplBackend = require('./templates/backend');
const tplFrontend = require('./templates/frontend');
const tplPrisma = require('./templates/prisma');
...
async function runSetup() {
    console.clear();
    console.log("-----------------------------------------");
    console.log(`   OMNISENDER v1.0.0 - INSTALLER`);
    console.log("-----------------------------------------");
    ...
    try {
        const files = [
            ...
            { path: 'reset-password.js', content: tplBackend.getResetScript() },
        ...
        // Se for update, não sobrescreve o .env para não perder configs
    ...
    try {
        ...
        console.log("-----------------------------------------");
        console.log("Para iniciar em Produção (Recomendado):");
        console.log(`${GREEN}npm start${RESET}`);
        console.log("\nPara iniciar em Desenvolvimento:");
        console.log(`${YELLOW}npm run dev${RESET}`);
        console.log("-----------------------------------------");
        
        const startProd = await ask("\nIniciar servidor agora (Modo DEV)? (s/n) > ");
        if(startProd.toLowerCase() === 's') execSync('npm run dev', { stdio: 'inherit' });
